...
const foreCast = (longitude,latitude, callback) =>{
    ...
    request({url, json : true},(error, response)=>{
        if (error) {
            ...
            callback(undefined,` ${response.body.hourly.summary} Currently ${response.body.currently.temperature} degrees out. There is  ${response.body.currently.precipProbability}%  chance of Rain`)
        }
    })
}
